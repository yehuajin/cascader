...
const convertToRows = (originColumns) => {
  ...
  const traverse = (column, parent) => {
    ...
    // 。。。。。新增
    if (column.levelNum) {
      column.level = column.levelNum
    } else if (parent) {
      column.level = parent.level + 1 + (parent.rowSpan ? parent.rowSpan - 1 : 0);
    }
    if (maxLevel < column.level) {
      maxLevel = column.level;
    }
    ...
    if (column.children) {
      ...
      column.colSpan = column.colSpan ? column.colSpan : colSpan; // 。。。。。新增
    } else {
      // column.colSpan = 1;
      column.colSpan = column.colSpan ? column.colSpan : 1; // 。。。。。新增
    }
  };
